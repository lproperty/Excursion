            ...
            {showServicesFloatPill && (
              <>
                <div class="service-flex">
                  <h1>
                    {`Showing ${routeServices.length} ${routeServices.length !== 1 ? 'services' : 'service'}`}
                  </h1>
                </div>
                <div class="services-list">
                  ...
                  {!!intersectStops.length && (
                    <>
                      <h2>
                        {`${intersectStops.length} ${intersectStops.length !== 1 ? 'intersecting stops' : 'intersecting stop'}`}
                      </h2>
            ...
            {showPassingRoutesFloatPill && (
              <>
                ...
                <div class="services-list" onClick={unhighlightRoute}>
                  <h2>
                    {`${stopsData[route.value].services.length} ${stopsData[route.value].services.length !== 1 ? 'passing routes' : 'passing route'}`}{' '}
                    ·{' '}
                    <a
      ...
      >
        ...
        {servicesData && routeServices.length && (
          <>
            ...
            >
              <h2>
                {`${servicesData[routeServices[0]].routes.length} ${servicesData[routeServices[0]].routes.length !== 1 ? 'routes' : 'route'}`}{' '}
                ∙{' '}
                {servicesData[routeServices[0]].routes
                  .map((route) => `${route.length} ${route.length !== 1 ? 'stops' : 'stop'}`)
                  .join(' ∙ ')}
                &nbsp;&nbsp;
                <button
                  type="button"
